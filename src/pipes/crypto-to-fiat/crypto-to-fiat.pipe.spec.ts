...
describe('CryptoToFiatPipe', () => {
  ...
  it('should return the right price', () => {
    expect(cryptoToFiatPipe.transform(new BigNumber(1), { protocolIdentifier: 'eth', currentMarketPrice: new BigNumber(200) })).toEqual(
      '0.0000000000000002'
    )
  })
  ...
  it('should return an empty string when protocolIdentifier is invalid', () => {
    expect(
      cryptoToFiatPipe.transform(new BigNumber(1), { protocolIdentifier: 'bananarama', currentMarketPrice: new BigNumber(200) })
    ).toEqual('')
  })

  it('should return an empty string when value is not a BigNumber', () => {
    let value: any = 'test'
    expect(cryptoToFiatPipe.transform(value, { protocolIdentifier: 'eth', currentMarketPrice: new BigNumber(200) })).toEqual('')
  })
