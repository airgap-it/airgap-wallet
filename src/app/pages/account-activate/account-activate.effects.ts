...
export class AccountActivateEffects {
  public initialData$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([action, walletGroups]) =>
        concat(of(actions.initialDataLoaded({ walletGroups })), of(this.loadNavigationData(action.routeParams)))
      )
    )
  )
  ...
  private loadNavigationData(routeParams: any): Action {
    const protocolIdentifier: ProtocolSymbols | undefined = routeParams.protocolID

    return protocolIdentifier !== undefined ? actions.navigationDataLoaded({ protocolIdentifier }) : actions.invalidNavigationData()
  }
