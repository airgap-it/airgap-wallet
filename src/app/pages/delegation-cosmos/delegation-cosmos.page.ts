...
export class DelegationCosmosPage {
  ...
  public async checkAddressDelegations() {
    ...
    if (index > -1) {
      ...
      this.delegatedAmount = rawDelegatedAmount.shiftedBy(
    ...
    this.delegatableBalance = rawDelegatableBalance.shiftedBy(
      -1 * this.wallet.coinProtocol.decimals
    )
    if (
      this.delegatableBalance <
      0 + this.wallet.coinProtocol.feeDefaults.low.toNumber()
    ) {
